...
class Authorization {
    ...
    private createJWTCookie(res: Response, id: number) {
        const signedString = jwt.sign({id:`${id}`}, cfg.JWT_SIGING_KEY, {
            expiresIn: cfg.JWT_EXPIRES*1000
        })
        res.cookie(authCookieName, signedString, {
            sameSite: 'strict',
            httpOnly: true,
            maxAge: cfg.JWT_EXPIRES*1000,
            secure: true,
        })
    }
    ...
    AuthCheck(req: Request) {
        ...
        try {
            const id = jwt.verify(token, cfg.JWT_SIGING_KEY) as jwt.JwtPayload
            return id.id
        }
        catch(e) {
            return 0
